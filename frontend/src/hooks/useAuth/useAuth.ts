import { AuthenticateData, RegisterData, authenticate, getProfile, refreshToken, signUp } from "../../api/auth";
    ...
    return {
        initialLoad: async () => {
            ...
            if (refresh) {
                ...
                if (response.data.access && response.data.refresh) {
                    ...
                    setData({ refresh: response.data.refresh, access: response.data.access, user: profile.data, isAuthenticated: true})
                } else {
        ...
        logIn: async (credentials: AuthenticateData) => {
            ...
            if (tokens.data.access && tokens.data.refresh) {
                const profile = await getProfile()
                setData({ refresh: tokens.data.refresh, access: tokens.data.access, user: profile.data, isAuthenticated: true})
            } else {
